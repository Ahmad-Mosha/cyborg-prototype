  ...
  return (
    <View className="flex-1 bg-dark-900">
      ...
      >
            ...
            <View className="flex-row justify-between items-center mb-4">
              <View className="flex-row items-center">
                ...
                <View>
                  ...
                  <Text className="text-gray-400">45 minutes · 5 exercises</Text>
                </View>
              </View>
        ...
        >
          <View className="flex-row justify-between items-center mb-4">
            <Text className="text-white text-lg font-bold">Weekly Progress</Text>
            <TouchableOpacity>
          ...
          <View className="bg-dark-800 rounded-3xl p-5 border border-dark-700">
            <View className="flex-row justify-between mb-4">
              <View>
                ...
                <Text className="text-white text-xl font-bold">12 <Text className="text-gray-400 text-sm font-normal">/ 16</Text></Text>
              </View>
              
              <View>
                <Text className="text-gray-400 mb-1">Calories Burned</Text>
                <Text className="text-white text-xl font-bold">2,450 <Text className="text-gray-400 text-sm font-normal">kcal</Text></Text>
              </View>
            </View>
                ...
                return (
                  <View key={index} className="items-center">
                    <View className="flex-1 w-10 justify-end">
                      <View 
                        className={`w-6 rounded-full ${isActive ? 'bg-primary' : 'bg-gray-600'}`} 
                        style={{ height: `${height}%` }} 
                      />
                    </View>
                    <Text className={`mt-2 ${isActive ? 'text-primary' : 'text-gray-400'}`}>{day}</Text>
                  </View>
                );
              })}
            </View>
        ...
        >
          <View className="flex-row justify-between items-center mb-4">
            <Text className="text-white text-lg font-bold">Your Achievements</Text>
            <TouchableOpacity>
              <Text className="text-primary">View All</Text>
            </TouchableOpacity>
          </View>
          ...
          >
            {[
              { title: 'Early Bird', desc: 'Complete 5 morning workouts', icon: 'sunny-outline', color: '#FFC107', progress: 80 },
              { title: 'Consistency', desc: 'Work out 7 days in a row', icon: 'calendar-outline', color: '#2196F3', progress: 60 },
              { title: 'Strength Master', desc: 'Lift 1000kg total in a session', icon: 'barbell-outline', color: '#4CAF50', progress: 30 },
              ...
              >
                <View className="flex-row items-center mb-3">
                  ...
                  <Text className="text-white font-bold" numberOfLines={1}>{achievement.title}</Text>
                </View>
                
                <Text className="text-gray-400 text-sm mb-3" numberOfLines={2}>{achievement.desc}</Text>
                ...
                <Text className="text-gray-400 text-xs mt-2 text-right">{achievement.progress}%</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        </Animated.View>
